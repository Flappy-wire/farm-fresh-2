        ...
        <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/60 p-0 backdrop-blur-sm sm:items-center sm:p-4">
          <div className="max-h-[94vh] w-full max-w-xl overflow-y-auto rounded-t-3xl bg-[#fffdf7] shadow-2xl sm:rounded-3xl">
            ...
            <div className="p-5">
              ...
              {checkoutStep === 2 && <div className="space-y-3"><div><h3 className="font-extrabold text-emerald-950">Delivery mode</h3><p className="text-xs text-stone-500">Pickup is free, or choose a farm logistics partner.</p></div><button onClick={() => setDeliveryMode("self")} className={`w-full rounded-xl border-2 p-4 text-left ${deliveryMode === "self" ? "border-emerald-700 bg-emerald-50" : "border-stone-200 bg-white"}`}><p className="font-bold text-emerald-950">Self pickup <span className="float-right text-emerald-700">Free</span></p><p className="mt-1 text-xs text-stone-500">Collect from the farm collection point.</p></button>{DELIVERY_PARTNERS.map((partner) => <button key={partner.id} onClick={() => { setDeliveryMode("partner"); setSelectedPartner(partner.id); }} className={`w-full rounded-xl border-2 p-4 text-left ${deliveryMode === "partner" && selectedPartner === partner.id ? "border-emerald-700 bg-emerald-50" : "border-stone-200 bg-white"}`}><p className="font-bold text-emerald-950">{partner.name} <span className="text-xs text-amber-600">★ {partner.rating}</span><span className="float-right text-emerald-700">{partner.fee ? `₹${partner.fee}` : "Free"}</span></p><p className="mt-1 text-xs text-stone-500">Estimated delivery: {partner.eta}</p></button>)}<div className="flex justify-between pt-2"><Button variant="outline" onClick={() => setCheckoutStep(1)}>Back</Button><Button onClick={() => setCheckoutStep(3)} className="bg-emerald-700 text-white hover:bg-emerald-800">Review order</Button></div></div>}
              {checkoutStep === 3 && <div className="space-y-4"><div><h3 className="font-extrabold text-emerald-950">Review your order</h3><p className="text-xs text-stone-500">One last check before we notify the farmers.</p></div><div className="rounded-xl border border-stone-200 bg-white p-4 text-sm"><p className="font-bold text-emerald-950">{cart.length} item{cart.length === 1 ? "" : "s"} · {cartTotalItems.toLocaleString()} kg</p><p className="mt-1 text-xs text-stone-600">{deliveryAddress.fullName}, {deliveryAddress.addressLine1}, {deliveryAddress.city}, {deliveryAddress.state} — {deliveryAddress.pincode}</p><p className="mt-2 text-xs font-bold text-emerald-800">{deliveryMode === "self" ? "Self pickup (free)" : `${chosenPartner?.name} · ${chosenPartner?.eta}`}</p></div><div className="rounded-xl bg-emerald-950 p-4 text-white"><div className="flex justify-between text-sm"><span>Farm produce</span><span>₹{cartTotalPrice.toLocaleString("en-IN")}</span></div><div className="mt-2 flex justify-between text-lg font-black"><span>Total</span><span>₹{(cartTotalPrice + deliveryFee).toLocaleString("en-IN")}</span></div></div><div className="flex justify-between"><Button variant="outline" onClick={() => setCheckoutStep(2)}>Back</Button><Button onClick={handleProceedCartCheckout} disabled={isCheckingOutCart} className="bg-[#f6b93b] font-black text-emerald-950 hover:bg-[#ffd163]">{isCheckingOutCart ? "Placing..." : "Place order"}</Button></div></div>}
            </div>
          </div>
        </div>
      )}
      ...
      {placedOrder && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4 animate-in zoom-in-95">
            ...
            {placedOrder.partner && <div className="rounded-xl bg-amber-50 p-3 text-xs text-emerald-950"><p className="font-black">{placedOrder.partner.name} · ★ {placedOrder.partner.rating}</p><p>{placedOrder.partner.vehicle} · ETA {placedOrder.partner.eta}</p><a className="mt-1 inline-flex items-center gap-1 font-bold text-emerald-700" href={`tel:${placedOrder.partner.contact}`}><Phone className="size-3" />Call delivery partner</a></div>}
